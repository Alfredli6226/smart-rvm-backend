...
const displayCO2 = computed(() => {
  // Show filtered data when a non-default date range is selected
  const isFiltered = envDateRange.value !== 'thisMonth';
  if (isFiltered) return (filteredCO2.value || 0).toFixed(1);
  return (totalWeight.value * CO2_PER_KG).toFixed(1);
});

const displayTrees = computed(() => {
  const isFiltered = envDateRange.value !== 'thisMonth';
  if (isFiltered) return filteredTrees.value || 0;
...
// Public viewer: not logged in but should see dashboard
const isLoggedIn = computed(() => !!auth.role);
...
<template>
  <div class="space-y-6 sm:space-y-8 p-3 sm:p-6 bg-gray-50 min-h-screen">
    ...
    <div v-show="!statsLoading">
      ...
      <div v-if="!isAgent && !isCollector || !isLoggedIn" class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-8">
      ...
      <div v-if="!isAgent && !isCollector || !isLoggedIn" class="mt-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
